...
A centroid exposes three component vectors plus an aggregate, available for similarity search:

| Component | Description |
|-----------|-------------|
| `face` | The aggregate position across all components |
| `beta` | Captures the most dynamic and expressive patterns of the persona |
| `delta` | Represents intermediate cognitive structures — more stable than beta, more active than epsilon |
| `epsilon` | Reflects the deepest, most fundamental and enduring foundations of the persona |
...
Returns pairwise cosine similarities between the listed faces across all components. All faces must be owned by you.
...
```bash
curl "https://api.faces.sh/v1/faces/FACE_ALIAS/neighbors?k=5&component=face&direction=nearest" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```json
{
  "data": {
    "neighbors": [
      { "alias": "bob", "similarity": 0.83 },
      { "alias": "carol", "similarity": 0.71 }
    ]
  }
}
```
