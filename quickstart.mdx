...
- API base URL: `https://api.faces.ai`
...
curl https://api.faces.ai/auth/nonce/0xYOUR_WALLET_ADDRESS
...
```bash
curl -X POST https://api.faces.ai/v1/faces \
...
```bash
curl -X POST https://api.faces.ai/v1/faces/face_abc123/upload \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
```bash
curl -X POST https://api.faces.ai/v1/faces/face_abc123/sync \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl https://api.faces.ai/v1/faces/face_abc123 \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST https://api.faces.ai/v1/chat/completions \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "alice",
