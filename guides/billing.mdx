...
| Plan | Price | Faces | Compile quota | OAuth chat routing |
|------|-------|-------|---------------|-------------------|
| Free | $0 | Unlimited | Pay-per-token | — |
| Connect | $17/mo | Unlimited | 1,000,000 tokens / 30 days | OpenAI |

Chat usage (token costs) is billed from your credit balance on all plans.

The **Connect** plan also lets you link your OpenAI account via OAuth, routing eligible chat requests through your ChatGPT subscription at no additional cost. See [OAuth connections](/guides/oauth) for details.

Compile interviews always use the system model — OAuth is not used for compilation.
