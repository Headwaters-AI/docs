...
On the **Connect** plan, you link your ChatGPT account via OAuth (device-code flow). Codex-eligible models (gpt-5.x) run free through your ChatGPT subscription **at `POST /v1/responses`**, and compile interviews run free too. Other usage — non-codex models, or codex models called at `/v1/chat/completions` — is billed per-token from your credit balance (the same rates as Free). If OAuth is unavailable (e.g., you hit OpenAI's limits), `/v1/responses` requests fall back to credits only when `api_fallback` is enabled. See [OAuth connections](/guides/oauth) for details.
