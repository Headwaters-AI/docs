...
You can reference any face in your messages using `${face-username}` syntax. Each `${...}` token is replaced with the face's display name in your message text, and the referenced face's profile is provided to the model as context. This lets you bring multiple faces into a single prompt.
...
You can pass a bare model name instead of `face@model`. No persona is injected — the model acts as a standard assistant. Templates still work, so you can inject face profiles as context:
...
Templates work in all proxy endpoints:
...
| `422` | Invalid request body or unknown face username in `${...}` template |
