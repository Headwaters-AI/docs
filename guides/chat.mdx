...
```bash
curl https://api.faces.ai/v1/models \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
# OpenAI proxy
curl -X POST https://api.faces.ai/v1/openai/chat/completions \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o",
...
# Anthropic proxy
curl -X POST https://api.faces.ai/v1/anthropic/messages \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
client = OpenAI(
    ...
    base_url="https://api.faces.ai/v1"
)
