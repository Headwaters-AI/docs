...
| `"failed"` | Error occurred — stop polling, check `error` field |
...
```bash
curl -X POST "https://api.faces.sh/v1/faces/FACE_USERNAME/upload?type=thread&face_speaker=Troy" \
  -H "Authorization: Bearer YOUR_API_KEY" \
...
Once uploaded, sync the thread to compile it into the knowledge graph:
